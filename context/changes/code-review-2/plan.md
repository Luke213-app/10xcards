...
- [x] 4.6 `reviews/ai-cr.md` committed without retriggering the workflow — 43c6b1d; note: the no-retrigger guarantee comes from the `GITHUB_TOKEN`-push rule + the author-based guard step, NOT from `[skip ci]` (which was dropped in `fix/ai-cr-label-retry-skip-ci` because it killed 4.7's label retry — see below).
- [x] 4.7 `ai-cr:review` label re-runs and updates in place — verified post-merge on throwaway PR #21 (now closed). Mechanism works: on a clean HEAD, adding `ai-cr:review` fired a `labeled` run within ~2s and updated the **same** marker comment in place (`comment_id` unchanged, exactly one marker comment, run link refreshed). `opened` trigger also confirmed firing (it had not in-session — the workflow simply wasn't on `master` yet). **The predicted 4.6↔4.7 tension was REAL:** when HEAD carried `[skip ci]` (the steady state, since 4.6's report commit landed `[skip ci]` on top), GitHub suppressed ALL `pull_request` events including `labeled` — adding the label produced **zero** runs over 2+ min. **Fixed** in `fix/ai-cr-label-retry-skip-ci`: dropped `[skip ci]` from the report commit; recursion stays prevented by the `GITHUB_TOKEN`-push rule + the author-based guard step.
