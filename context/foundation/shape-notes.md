...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "primary persona"
      decision: "professional upskiller — time-poor, learning for work, brings own source material"
    - topic: "core insight"
      decision: "workflow friction; user already values SR, manual card creation is the wall that kills motivation"
    - topic: "auth strategy"
      decision: "email + password login accounts; server-stored decks"
    - topic: "role model"
      decision: "flat — every user identical, manages only their own decks; no admin/guest roles"
    - topic: "MVP flow"
      decision: "login → paste text → generate → review candidates (accept/edit/reject) → save → spaced-repetition review"
    - topic: "SR algorithm choice"
      decision: "integrate a ready-made algorithm (not build own); specific library deferred — see Open Questions"
  frs_drafted: 11
...
- ≥ 75% of AI-generated flashcards are accepted by users (generation quality bar — from seed).
- ≥ 75% of all flashcards a user creates are made via AI rather than manually (the AI path is the preferred one — from seed).
